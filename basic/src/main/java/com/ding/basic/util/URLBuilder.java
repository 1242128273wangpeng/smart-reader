...
public class URLBuilder {
    ...
    public static String buildUrl(String host, String uriTag, Map<String, String> params) {
        params = EncodeMap(params);
        uriTag = encodeUriTag(uriTag);
        String token = Token.Companion.loadRequestToken(uriTag, params);
        String encode = "";
        try {
    ...
    private static Map<String, String> EncodeMap(Map<String, String> map) {
...
//	Pattern p = Pattern.compile("((\\w+\\.)?quanbennovel\\.com(:\\d+)?)|((\\w+\\.)?caotangteach\\.com(:\\d+)?)|((\\w+\\.)?lzbook\\.net(:\\d+)?)|((\\w+\\.)?wubutianxia\\.com(:\\d+)?)|((\\w+\\.)?youshiit\\.com(:\\d+)?)|((\\w+\\.)?dushixiaoshuo\\.cn(:\\d+)?)|((\\w+\\.)?lsread\\.cn(:\\d+)?)|((\\w+\\.)?0106636\\.cn(:\\d+)?)|((\\w+\\.)?zhuishuwang\\.com(:\\d+)?)|((\\w+\\.)?bookapi\\.cn(:\\d+)?)|((test1\\.api\\.)?bookapi\\.cn(:\\d+)?)|((127\\.0\\.0\\.1)(:\\d+)?)");
    ...
    public static String buildContentUrl(String url, Map<String, String> params) {
        params = EncodeMap(params);
        return urlFilter(url, params);
    }

    private static String urlFilter(String url, Map<String, String> paramsMap) {
        URL u = null;
        try {
            u = new URL(url);
        ...
        if ("/v3/book/chaptersContents".equals(u.getPath())) {
            Map<String, String> urlParams = Token.Companion.loadRequestParameters(u.getQuery());
            urlParams.putAll(paramsMap);
            String token = Token.Companion.loadRequestToken(u.getPath(), urlParams);
            String encode = "";
            try {
                encode = URLEncoder.encode(token, "UTF-8");
            } catch (UnsupportedEncodingException e) {
    ...
    private static Map<String, String> urlParamsToMap(String postParam) {
        String p = "&" + postParam;
        String urlKey = "&(\\w+)=";
    ...
    public static Map<String, String> getParamsByUrl(URL url) {
