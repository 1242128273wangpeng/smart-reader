...
import net.lzbook.kit.user.bean.QGBook;
...
public class QGRecommendBookStrategy extends RecommendBookStrategy {
    ...
    public QGRecommendBookStrategy(RecommendBooksResp bean) {
        bookBeans = bean;
        if (bookBeans != null) {
            getBookFromList();
        }
    ...
    public QGRecommendBookStrategy(RecommendBooksEndResp bean) {
        mBooksEndBean = bean;
     ...
     * 获取书末推荐位的青果书籍1
     */
    public void getQGBook1FromList() {
     ...
     * 获取书末推荐位的青果书籍1
     */
    public void getQGBook2FromList() {
        RecommendBooksEndResp.DataBean data = mBooksEndBean.getData();
        if (data != null) {
            RecommendBooksEndResp.DataBean.MapBean map = data.getMap();
            if (map != null) {
