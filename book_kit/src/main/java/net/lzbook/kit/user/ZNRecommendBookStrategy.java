...
import net.lzbook.kit.user.bean.ZNBook;
...
public class ZNRecommendBookStrategy extends RecommendBookStrategy {
    ...
    public ZNRecommendBookStrategy(RecommendBooksResp bean) {
        bookBeans = bean;
        if (bookBeans != null) {
            getBookFromList();
        }
    ...
    public ZNRecommendBookStrategy(RecommendBooksEndResp bean) {
        mBooksEndBean = bean;
    ...
    public void getBookFromBookendList() {
    ...
    Book getRecommendBook() {
        ...
        while (mZnListBooksList.size() != 0) {
            ...
            if (!mDislikeBooksList.contains(selectedBook.getBookId())) {
                return new ZNBook.Builder().bookId(selectedBook.getBookId()).bookName(selectedBook.getBookName()).bookSourceId(selectedBook.getId()).author(selectedBook.getAuthorName())
                        .category(selectedBook.getLabel()).chapterCount(selectedBook.getChapterCount()).host(selectedBook.getHost()).lastChapterName(selectedBook.getLastChapterName())
                        .imgUrl(selectedBook.getSourceImageUrl()).updateTime(selectedBook.getUpdateTime()).build();
            }
        }
        return null;
    }
    ...
    Book getBookendRecommendBook() {
        ...
        if (mBookendBooksList != null && mBookendBooksList.size() != 0) {
            ...
            if (znBook != null) {
                return new ZNBook.Builder().bookId(znBook.getBookId()).bookName(znBook.getBookName()).bookSourceId(znBook.getId()).author(znBook.getAuthorName())
                        .category(znBook.getLabel()).chapterCount(znBook.getChapterCount()).host(znBook.getHost()).lastChapterName(znBook.getLastChapterName())
                        .imgUrl(znBook.getSourceImageUrl()).updateTime(znBook.getUpdateTime()).dex(znBook.getDex()).build();
            }
        }
        return null;
    }
