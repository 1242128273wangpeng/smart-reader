...
public class BookShelfRemoveHelper {
     ...
     */
    public void showRemoveMenu(View parent, int position) {//显示菜单
        if (popupWindow != null && popupWindow.isShowing()) {
            onShowing(false);
            popupWindow.dismiss();
    ...
    private void setRemoveWindow(Context context) {//实例化菜单对象
        ...
        View baseView = LayoutInflater.from(context).inflate(net.lzbook.kit.R.layout.download_manager_bottom, null);
        popupWindow = new PopupWindow(baseView, WindowManager.LayoutParams.MATCH_PARENT, (int) context.getResources().getDimension(


                net.lzbook.kit.R.dimen.dimen_view_height_default));
        popupWindow.setAnimationStyle(net.lzbook.kit.R.style.remove_menu_anim_style);
        delete_btn = (Button) baseView.findViewById(net.lzbook.kit.R.id.btn_right);
        selectAll_btn = (Button) baseView.findViewById(net.lzbook.kit.R.id.btn_left);
        ViewGroup layout = (ViewGroup) baseView.findViewById(net.lzbook.kit.R.id.remove_delete_layout);
    ...
    private void setDeleteNum() {
        if (bookShelfReAdapter != null && delete_btn != null && selectAll_btn != null) {
            ...
            if (num == 0) {
                ...
                int textCsl = mContext.getResources().getColor(net.lzbook.kit.R.color.color_gray_babfc1);
                delete_btn.setTextColor(textCsl);
                delete_btn.setBackgroundResource(R.drawable.bookshelf_delete_submit_default_bg);
            } else {
                delete_btn.setText("删除 (" + num + ")");
                ColorStateList draw = mContext.getResources().getColorStateList(R.color.bookshelf_delete_submit_text_color);
                delete_btn.setTextColor(draw);
                delete_btn.setBackgroundResource(R.drawable.bookshelf_delete_submit_bg);
            }
        }
    }
