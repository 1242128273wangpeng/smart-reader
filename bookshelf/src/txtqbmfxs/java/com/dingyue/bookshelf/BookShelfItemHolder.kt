...
import kotlinx.android.synthetic.txtqbmfxs.layout_bookshelf_item_grid.view.*
...
class BookShelfItemHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    fun bind(book: Book, bookshelfItemListener: BookShelfAdapter.BookShelfItemListener,
        ...
        // 是否连载
        if (book.status === 2) {
            this.txt_book_last_chapter.text = "已完结"
        } else {
        ...
        if (book.update_status != 1) {
            this.img_book_status.visibility = View.GONE
        } else {
            this.img_book_status.visibility = View.VISIBLE
        }
        ...
        if (!TextUtils.isEmpty(book.img_url) && !book.img_url.equals(ReplaceConstants.getReplaceConstants().DEFAULT_IMAGE_URL)) {
            Glide.with(itemView.context.applicationContext).load(book.img_url).placeholder(R.drawable.icon_book_cover_default).error(R.drawable.icon_book_cover_default).diskCacheStrategy(DiskCacheStrategy.ALL).into(this.img_book_cover)
        } else {
            Glide.with(itemView.context.applicationContext).load(R.drawable.icon_book_cover_default).into(this.img_book_cover)
        }
