...
//    }
     ...
     * 按钮左右偏移量
     */
    private static final int BUTTON_OFFSET = 0;
    ...
    private static float mdp2px(float dp) {
...
//        buttonOffset = (int) (BUTTON_OFFSET * density + 0.5f);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        left = w / 2 - (lineHeight);
        top = viewPadding;
        right = w / 2 + (lineHeight);
        bottom = h - viewPadding;

        centerX = (left + right) * .5f;
        centerY = (top + bottom) * .5f;

        buttonMinX = left + (lineHeight / 2) + buttonOffset;
        buttonMaxX = right - (lineHeight / 2) - buttonOffset;
...
//                paint);
        ...
        //绘制按钮
        drawRoundRect(canvas,
                viewState.buttonX - (lineHeight / 2), btnHalfHeightY - (lineHeight / 2),
                viewState.buttonX + (lineHeight / 2), btnHalfHeightY + (lineHeight / 2),
