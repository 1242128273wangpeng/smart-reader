...
import static net.lzbook.kit.utils.ExtensionsKt.setDebounceClickListener;
        ...
        if (bean.getData().getMap().getZnList() != null
            ...
            for (int i = 0; i < znSize; i++) {//推荐位 智能只取 3本
                ...
                CoverRecommendBean.DataBean.MapBean.ZnListBean znBean =
                        bean.getData().getMap().getZnList().get(znIndex);
        ...
        if (bean.getData().getMap().getQgList() != null
            ...
            for (int i = 0; i < qgSize; i++) {//推荐位 青果只取 3本
                ...
                CoverRecommendBean.DataBean.MapBean.QgListBean qgBean =
                        bean.getData().getMap().getQgList().get(qgIndex);
...
//        bookVoBean.readerCountDescp = bookMode.model.re
    ...
    private void upDateUI() {
        ...
        if (bookVo != null) {
            ...
            if (tv_text_number != null && bookVo.wordCountDescp != null) {
                if (Constants.QG_SOURCE.equals(bookVo.host) && !AppUtils.isContainChinese(
                        bookVo.wordCountDescp)) {
                    tv_text_number.setText(
                            AppUtils.getWordNums(Long.valueOf(bookVo.wordCountDescp)));
            ...
            if (tv_read_num != null && bookVo.readerCountDescp != null) {
                if (Constants.QG_SOURCE.equals(bookVo.host) && !AppUtils.isContainChinese(
                        bookVo.readerCountDescp)) {
                    tv_read_num.setText(
                            AppUtils.getReadNums(Long.valueOf(bookVo.readerCountDescp)));
                } else {
            ...
            if (flowlayout != null && !TextUtils.isEmpty(bookVo.labels)) {
                flowlayout.setChildSpacing(
                        getResources().getDimensionPixelOffset(R.dimen.cover_book_flowlayout));
            ...
            if (book_cover_update_time != null) {
                book_cover_update_time.setText(
                        Tools.compareTime(AppUtils.formatter, bookVo.update_time) + "更新");
            }
                ...
                if (bookSourceList != null && bookSourceList.size() == 0) {
                    if (requestItem != null && Constants.QG_SOURCE.equals(requestItem.host)) {
                        ...
                        requestItem.fromType =
                                1;// 打点统计 当前页面来源，所有可能来源的映射唯一字符串。书架(0)/目录页(1)/上一页翻页(2)/书籍封面(3)
                    } else {
                    ...
                    if (bookSourceList != null && bookSourceList.size() == 0) {
                        ...
                        if (bookDaoHelper != null && bookCoverUtil != null) {
                            if (!bookDaoHelper.isBookSubed(requestItem.book_id)) {
                                ...
                                boolean succeed = bookDaoHelper.insertBook(insertBook);
                                if (succeed && book_cover_bookshelf != null) {
                                    book_cover_bookshelf.setText(
                                            R.string.book_cover_havein_bookshelf);
                                    setRemoveBtn();
                                    showToastShort(getString(R.string.succeed_add));
                                    BaseBookHelper.startDownBookTask(CoverPageActivity.this,
                                            requestItem.toBook(), 0);
    ...
    public void onScrollChanged(int top, int oldTop) {
        if (AppUtils.px2dip(this, top) > 36 && bookVo != null) {
