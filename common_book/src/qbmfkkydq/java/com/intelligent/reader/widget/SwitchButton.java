...
//    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        lineHeight = (int) (LINE_HEIGHT * density + 0.5f);

        buttonOffset = (int) (BUTTON_OFFSET * density + 0.5f);

        TypedArray typedArray = null;
        if (attrs != null) {
            typedArray = context.obtainStyledAttributes(attrs, net.lzbook.kit.R.styleable.SwitchButton);
        }

        shadowEffect = optBoolean(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_shadow_effect,
                true);

        uncheckCircleColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_uncheckcircle_color,
                0XffAAAAAA);//0XffAAAAAA;

        uncheckCircleWidth = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_uncheckcircle_width,
                dp2pxInt(1.5f));//dp2pxInt(1.5f);

        uncheckCircleOffsetX = dp2px(10);

        uncheckCircleRadius = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_uncheckcircle_radius,
                dp2px(4));//dp2px(4);
        ...
        shadowRadius = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_shadow_radius,
                dp2pxInt(2.5f));//dp2pxInt(2.5f);

        shadowOffset = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_shadow_offset,
                dp2pxInt(1.5f));//dp2pxInt(1.5f);

        shadowColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_shadow_color,
                0X33000000);//0X33000000;

        uncheckedColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_uncheck_color,
                0XffDDDDDD);//0XffDDDDDD;

        checkedColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_checked_color,
                0Xff51d367);//0Xff51d367;

        uncheckedBtnColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_uncheckedbtn_color,
                0Xff51d367);//0Xff51d367;

        checkedBtnColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_checkedbtn_color,
                0Xff51d367);//0Xff51d367;

        borderWidth = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_border_width,
                dp2pxInt(1));//dp2pxInt(1);

        checkLineColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_checkline_color,
                Color.WHITE);//Color.WHITE;

        checkLineWidth = optPixelSize(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_checkline_width,
                dp2pxInt(1f));//dp2pxInt(1.0f);

        checkLineLength = dp2px(6);

        int buttonColor = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_button_color,
                Color.WHITE);//Color.WHITE;

        int effectDuration = optInt(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_effect_duration,
                300);//300;

        isChecked = optBoolean(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_checked,
                false);

        showIndicator = optBoolean(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_show_indicator,
                true);

        background = optColor(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_background,
                Color.WHITE);//Color.WHITE;

        enableEffect = optBoolean(typedArray,
                net.lzbook.kit.R.styleable.SwitchButton_sb_enable_effect,
                true);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isChecked()) {
            buttonPaint.setColor(checkedBtnColor);
            setCheckedViewState(viewState);
        } else {
            buttonPaint.setColor(uncheckedBtnColor);
...
//                viewRadius, paint);
        ...
        //绘制开启背景色
        float des = 0;//[0-backgroundRadius*0.5f]
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(viewState.checkStateColor);
//        paint.setStrokeWidth(borderWidth + des * 2f);
        int btnHalfHeightY = (int) ((bottom - top) / 2 + top);
        drawRoundRect(canvas,
                left, btnHalfHeightY - (lineHeight / 2),
                right, btnHalfHeightY + (lineHeight / 2),
                viewRadius, paint);
