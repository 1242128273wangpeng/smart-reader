...
class CataloguesActivity : BaseCacheableActivity(), OnClickListener, CataloguesContract, CallBackDownload {
    ...
    private fun initData(bundle: Bundle) {
        ...
        if (book != null) {
            txt_catalog_name!!.text = book!!.name
    ...
    private fun changeSortState(negative: Boolean) {
        if (negative) {
            img_catalog_sort.setImageResource(R.drawable.catalog_sort_negative_icon)
            StatServiceUtils.statAppBtnClick(this, StatServiceUtils.rb_catalog_click_zx_btn)
        } else {
            img_catalog_sort.setImageResource(R.drawable.catalog_sort_positive_icon)
    ...
    override fun requestCatalogSuccess(chapterList: ArrayList<Chapter>) {
        ...
        txt_catalog_count?.text = MessageFormat.format("共{0}章", chapterList.size)
    ...
    override fun insertBookShelfResult(result: Boolean) {
        if (result) {
            ...
            txt_catalog_shelf.setTextColor(Color.parseColor("#B9B9B9"))
        } else {
            txt_catalog_shelf.setTextColor(Color.parseColor("#42BE54"))
