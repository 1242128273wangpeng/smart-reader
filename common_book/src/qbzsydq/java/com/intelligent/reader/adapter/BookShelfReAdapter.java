...
public class BookShelfReAdapter extends RecyclerView.Adapter<AbsRecyclerViewHolder<Book>> {
    ...
    public void onBindViewHolder(AbsRecyclerViewHolder<Book> holder, int position) {
        ...
        if (position >= book_list.size()) {
            position = book_list.size() - 1;
        }
        Book book = book_list.get(position);
        switch (getItemViewType(position)) {
            case 0:
                holder.onBindData(position, book, update_table.contains(book.getBook_id()), isRemoveMode(), remove_checked_states.contains(position));
                break;
            case 1:
    ...
    private View getAdView(Book book) {
        ...
        if (book.getSequence() < mAdViews.size()) {
            return mAdViews.get(book.getSequence());
        }
        return null;
    }
    ...
    public int getItemViewType(int position) {
        if (book_list != null && position >= 0 && position <= book_list.size() - 1) {
            Book book = book_list.get(position);
            if (book != null) {
                if (book.getItem_type() == 0) {
                    return 0;
                } else if (book.getItem_type() == -2) {
                    return 1;
                }
            }
        }
