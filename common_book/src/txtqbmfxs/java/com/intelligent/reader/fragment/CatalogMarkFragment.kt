...
import kotlinx.android.synthetic.txtqbmfxs.item_read_bookmark.view.*
...
//        view.bookmark_main.layoutManager = LinearLayoutManager(activity)
        ...
        view.iv_fixbook.setOnClickListener { _ ->
            try {
        ...
        view.catalog_main.layoutManager = object : LinearLayoutManager(activity, LinearLayoutManager.VERTICAL, false) {
        ...
        view.bookmark_main.layoutManager = object : LinearLayoutManager(activity, LinearLayoutManager.VERTICAL, false) {
            override fun onLayoutChildren(recycler: RecyclerView.Recycler?, state: RecyclerView.State) {
                super.onLayoutChildren(recycler, state)
                //TODO if the items are filtered, considered hiding the fast scroller here
                val firstVisibleItemPosition = findFirstVisibleItemPosition()
...
//            view.iv_catalog_novel_sort.setImageResource(sortIcon.resourceId)
    ...
    override fun onDestroy() {
        super.onDestroy()
        (presenter as CatalogMarkPresenter).clearDisposable()
    }
