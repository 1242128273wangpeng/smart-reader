...
import kotlinx.android.synthetic.mfxsqbyd.item_read_bookmark.view.*
...
//                        img_catalog_order.visibility = View.VISIBLE
    ...
    private fun initListener() {
        ...
        img_book_cover.setOnClickListener {
            val bundle = Bundle()
    ...
    override fun showCatalog(chapters: List<Chapter>, sequence: Int) {
        ...
        if (ReaderStatus.book.status == "FINISH") {
            catalog_book_state.text = "已完结"
        } else {
            catalog_book_state.text = "连载中"
        }

        if (!TextUtils.isEmpty(ReaderStatus.book.img_url) && ReaderStatus.book.img_url != ReplaceConstants.getReplaceConstants().DEFAULT_IMAGE_URL) {
            Glide.with(catalog_book_state.getContext().getApplicationContext())
                    .load(ReaderStatus.book.img_url)
                    .placeholder(R.drawable.reader_book_cover_default)
                    .error(R.drawable.reader_book_cover_default)
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
                    .into(img_book_cover)
        } else {
            Glide.with(img_book_cover.getContext().getApplicationContext())
                    .load(R.drawable.reader_book_cover_default)
                    .into(img_book_cover)
        }
