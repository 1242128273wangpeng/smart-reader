...
public class LogClient {
    ...
    public LogClient(String endPoint, String accessKeyID, String accessKeySecret) {
        mHttpType = "http://";
        if (!"".equals(endPoint)) mEndPoint = endPoint;
        ...
        if (!"".equals(accessKeyID)) mAccessKeyID = accessKeyID;
        else throw new NullPointerException("accessKeyID is null");

        if (!"".equals(accessKeySecret)) mAccessKeySecret = accessKeySecret;
    ...
    public void postLog(final LogGroup logGroup, String logStoreName) throws LogException {
        ...
        byte[] httpPostBody;
        try {
            httpPostBody = logGroup.toJsonString().getBytes("UTF-8");
    ...
    public void HttpPostRequest(final List<ServerLog> logList, String url, Map<String, String> headers, byte[] body) throws LogException {
        ...
        try {
            ...
            AndroidLogStorage.INSTANCE.consumeSuccess(logList);
        } catch (IOException e) {
